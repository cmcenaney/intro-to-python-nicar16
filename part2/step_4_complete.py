...
def more_exploring_with_strings(file_name):
    ...
    print 'I made %d strings from a list. They are %s and %s' % (len(my_split_string), my_split_string[0].lower(), my_split_string[1].lower())

    # Close the csv file when we're done
    csv_file.close()

# Call the function, passing as an argument the name of the csv file to open.
more_exploring_with_strings('../data/marijuana_gross_sales.csv')
